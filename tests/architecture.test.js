...
describe('Vite package architecture', () => {
  ...
  it('should keep the root SPA entry free of server and node imports', () => {
    for (const file of ['src/index.ts', 'src/jsx-plugin.ts', 'src/template-optimizer.ts']) {
      expect(readFileSync(resolve(root, file), 'utf8'), file).not.toMatch(/@askrjs\/(?:server|node)/);
    }
  });

  it('should delegate Node transport handling to askr-node', () => {
    expect(readFileSync(resolve(root, 'src/server/plugin.ts'), 'utf8')).toContain("from '@askrjs/node'");
  });

  it('should keep production modules within 300 lines', () => {
    const sourceFiles = [
      ...readdirSync(resolve(root, 'src')).filter((file) => file.endsWith('.ts')).map((file) => resolve(root, 'src', file)),
      ...readdirSync(resolve(root, 'src/server')).filter((file) => file.endsWith('.ts')).map((file) => resolve(root, 'src/server', file)),
    ];
