...
function moduleTelemetry(module: Record<string, unknown>) {
  ...
  return telemetry && typeof telemetry === "object"
    ? {
        telemetry: telemetry as NonNullable<
          Parameters<typeof composeAskrDocumentResponse>[2]
        >["telemetry"],
      }
    : undefined;
}

export function createDevelopmentApp(server: ViteDevServer, options: AskrServerOptions): ServerApp {
  return {
    async fetch(request) {
